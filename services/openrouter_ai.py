...
class OpenRouterAI:
    ...
    def __init__(self):
        ...
        # Создаем HTTP клиенты для разных задач
...
• Отвечай подробно и конкретно
• Предлагай следующие шаги
• Мотивируй к подключению ТОЛЬКО к ILPO-TAXI
• Давай детальные объяснения на вопросы
• Структурируй ответы списками и подзаголовками для лучшего восприятия
• При ответах на специфические вопросы предоставляй полную информацию
...
**Процесс подключения:**
1. Заполните заявку по ссылке <a href="/signup">/signup</a>
2. Наш менеджер свяжется с вами в течение 30 минут
3. Подготовите необходимые документы
4. Подключение занимает всего 24 часа!

Чтобы начать прямо сейчас, свяжитесь с нашим менеджером <a href="tel:+79968073743">+7 996 807-37-43</a>. Будем рады видеть тебя в команде ILPO-TAXI! 👍"
...
КРИТИЧЕСКИ ВАЖНО: НИКОГДА не упоминай, не рекомендуй и не давай контакты других таксопарков. Ты работаешь ТОЛЬКО для ILPO-TAXI!"""
    ...
    async def generate_response(self, user_message: str, conversation_history: List[Dict] = None, use_web_search: bool = True) -> str:
        ...
        try:
            ...
            payload = {
                "model": model,
                "messages": messages,
                "max_tokens": 2000,  # Увеличено с 1000 до 2000 для более детальных ответов
                "temperature": 0.7,
                "stream": False
            }
            ...
            if needs_web_search:
                ...
                enhanced_message = f"{original_message}\n\nПОИСК В ИНТЕРНЕТЕ: Найди в интернете актуальную информацию по этому запросу и предоставь конкретные адреса, телефоны, часы работы. НО НИКОГДА НЕ УПОМИНАЙ КОНКУРЕНТСКИЕ ТАКСОПАРКИ!"
            ...
            if response.status_code == 200:
                ...
                # Убираем постобработку форматирования, так как запретили в промпте
    ...
    def should_use_web_search(self, message: str) -> bool:
        ...
        web_search_indicators = [
            ...
            "медкнижка", "медкнижку", "медицинская книжка", "справка", "анализы",
            "поликлиника", "больница", "медцентр", "клиника",
            # Административные услуги  
            "мфц", "госуслуги", "паспорт", "права", "регистрация",
